...
const MetricCard = ({ title, value, valueUnit, icon, tooltip, iconColor }: MetricCardProps) => {
  return (
    <TooltipProvider>
      <Tooltip>
        <TooltipTrigger asChild>
          <div className="relative p-6 border rounded-xl bg-white shadow-sm hover:shadow-md transition-shadow duration-200">
            ...
            <div className="absolute -top-3 right-4 p-2 rounded-lg bg-gray-50 border shadow-sm">
              <div className="text-gray-500" style={{ color: iconColor || "inherit" }}>{icon}</div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <span className="ml-1 text-sm font-medium text-gray-500">{valueUnit}</span>
              </div>
            </div>
          </div>
        </TooltipTrigger>
